...
import { spawn, exec, execFile, spawnSync, execSync, execFileSync } from 'child_process'
import express from 'express'
import notify from './notify.js'

const __dirname = dirname(fileURLToPath(import.meta.url))
const include = async file => await Promise.resolve(await eval(readFileSync(file).toString()))
...
let browser, tunnel, page, server
...
app.use('/devtools', async (_req, res) => {
  res.redirect(await tunnel.getUrlForPage(page))
})
...
const newBrowser = async () => {
  try {
    ...
    tunnel = await devtools.createTunnel(browser)
...
export default () => new Promise(resolve => {
  newBrowser().then(() => {
    server = app.listen(port, () => {
      resolve({ browser, tunnel, page, server })
    })
  })
})
