...
function WebSocket(host, options) {
  ...
  this.key = {
    source: randomString,
    hashed: btoa(require("crypto").SHA1(randomString + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"))
  };
}
...
WebSocket.prototype.send = function(msg, opcode) {
  ...
  this.socket.write(String.fromCharCode(opcode, size + (this.masking ? 128 : 0)));

  if (size == 126) {
    // Need to write extra bytes for longer messages
    this.socket.write(String.fromCharCode(msg.length >> 8));
    this.socket.write(String.fromCharCode(msg.length));
  }
  ...
  if (this.masking) {
    ...
    for (var ix = 0; ix < 4; ix++) {
      ...
      mask[ix] = rnd;
      masked += String.fromCharCode(rnd);
    }
    for (var iy = 0; iy < msg.length; iy++)
      masked += String.fromCharCode(msg.charCodeAt(iy) ^ mask[iy & 3]);
  ...
  var server = require("http").createServer(function(req, res) {
    if (req.headers.Connection && req.headers.Connection.indexOf("Upgrade") >= 0) {
      ...
      if (req.headers["Sec-WebSocket-Protocol"]) {
        resHeaders["Sec-WebSocket-Protocol"] = req.headers["Sec-WebSocket-Protocol"];
      }
